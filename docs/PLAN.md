...
| 页面 | 原因 | 建议 |
|---|---|---|
| `index.html` | JS 驱动的轮播，96 词无法静态还原 | 需要一个 `carousel` 类型化区块，或手工转换 |
| `blog/index.html` | 含清洗器会剥离的标记 | 博客索引本就是动态列表，更适合 `article_list` 区块 |
| `404.html` | 没有 `data-page-body` 挂载点 | 加一个挂载点即可转换 |

这三个都需要**新的类型化区块**而不是 `rich_text` 兜底——正是区块系统设计上
该做的事，一个文件一个类型。
...
- [x] 阶段 3 — 静态页迁移（24/27 页，16,771 词零丢失，75 条测试全绿）
