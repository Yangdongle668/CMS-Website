...
### 8. 前台没有设计系统（后台有）— ✅ 令牌层已完成

按 `PLAN.md` 步骤 3 收窄了范围：**只做会被区块系统继承的令牌层**，
不做 4498 行的全量重构（那些页面级 CSS 会随 Phase 3 的页面重写一并处理）。

结果：`:root` 令牌 **0 → 19 个**，`var()` **9 → 421 处**，硬编码色 **478 → 63 处**，
并用完整 `diff` 证明了展开后逐字节一致（零视觉回归）。详见 `PLAN.md` 步骤 3。
...
**结果**：回填后 seed 图片 16.9 MB → 4.4 MB（−74%），
服务端渲染页面的图片体积 −76%。`server/services/image-render.js` 成为共享渲染器，
今天的 SSR 层与区块的 `render()` 都走它，第 3 点里说的 `background-image`
也用 `image-set()` 做了格式协商（AVIF / WebP / JPEG，带 `url()` 兜底声明）。

<details>
<summary>原始分析（含一处已更正的错误前提）</summary>
...
### 10. 响应式断点碎片化 — ⚠️ 原方案不可实现，已更正；**未改，理由见下**

现状核实：`public/styles.css` 共 46 条 `@media`，用了 11 个不同阈值——
`480 · 520 · 600 · 700 · 760 · 800 · 900 · 1000 · 1100 · 1200`（外加一条 `min-width: 1101px`）。
确实是不同时期叠加出来的。

**更正一：原文写的「定义为 CSS 变量统一引用」在纯 CSS 里做不到。**
`var()` 只在属性值里有效，媒体查询的条件部分不支持它——整条 at-rule 会被判为无效直接丢弃。
已实测（Chromium 141，900px 视口）：

| 写法 | 900px 下是否生效 |
|---|---|
| `@media (max-width: var(--bp-lg))` | ❌ 不生效（规则被丢弃） |
| `@media (max-width: 1100px)` | ✅ 生效 |

真要做到「一处定义、处处引用」，只有两条路：引入构建期预处理（PostCSS / Sass），
或者等 `@custom-media`（Media Queries Level 5 的草案，目前无浏览器支持）。
**这是给这个项目加一层构建依赖的决定，不该顺手做掉。**

**更正二：`PLAN.md` 里「页面级媒体查询会随页面重写」这个前提没有成立。**
阶段 3/4 把 25 个页面转成了区块，但区块渲染进的是**同一批 class**，
`styles.css` 一行没动。所以这些断点不会被自然淘汰——它们会一直在。

**为什么现在仍然不改**：剩下的部分（把 11 个阈值手工收敛成 4 个）是纯维护性收益，
而代价是**真实的视觉回归风险且此处无法验证**——把 `700px` 挪到 `768px`，
就是在改 700–768px 之间所有设备上的布局，而这个仓库没有视觉回归测试。
建议的前置条件：先有截图对比（Chromium 已装，可跑），再动阈值。

**顺带排除一个误报**：`max-width: 1100px` 与 `min-width: 1101px` 这一对看着像
差一像素的缝（视口宽度在缩放下可以是小数，比如 1100.5px）。实测**不成立**——
在 1100.00…1101.00 之间取 101 个小数宽度，没有任何一个宽度两条规则都不匹配：
Chromium 在求值前把视口宽度取整了。已改回原样，不要再查这条。

### 11. 焦点样式 — ⚠️ 本条原判断有误，已更正；全局部分已完成

**更正**：原文说「`:focus-visible` 各仅出现 1 次，键盘用户在多数交互元素上看不到焦点指示」
——**不成立**。那 1 次是 `public/styles.css:2018` 的**通用选择器**
（`:focus-visible { ... }`，无前缀），覆盖所有可聚焦元素。我当时只统计了出现次数，
没有看选择器的作用范围。

**真实问题是对比度，已实测并修复**：原焦点环用琥珀 `#f5a623`，
在白底上仅 **2.03:1**，低于 WCAG 2.2 SC 1.4.11 对焦点指示器要求的 3:1。
已改为双色环（详见 `PLAN.md` 步骤 3），浅色背景 17.43:1、深色区块 9.32:1。

**仍待设计定夺**：`--text-mute3` (#a1a1a6) 在浅色背景上是 **2.57:1**，
低于正文要求的 4.5:1（`styles.css:4490`、`:4505` 的小字）。改它是设计决策，未擅自动。
...
## P3 — 工程化（决定前面所有项改完敢不敢部署）— ✅ 已完成

**结果**：88 条集成测试（真服务器 + 每次运行独立数据库）、ESLint 10 扁平配置、
GitHub Actions 三个 job（lint / test / audit）。

下面是当时的原始状态，留作对照：

- ~~**零测试** — 全仓库无 `.test.js` / `.spec.js`~~ → 7 个测试文件，88 条断言
- ~~**零 CI** — 无 `.github/` 目录~~ → `.github/workflows/ci.yml`
- ~~**零 lint/format** — 无 ESLint、Prettier、EditorConfig~~ → 三者齐备
...
| **第 3 周** | #8 前台 token 系统 · #7 索引优化 · #10 断点收敛 | 3 天 | #8 #7 ✅；#10 见上，方案本身不成立 |
| **第 4 周** | #11 无障碍检查 · P3 工程化全套 | 3 天 | ✅（#11 余一个颜色待设计定夺）|
...
清单上**没有一条是「还没做」**，只有两条是「需要你先做决定」：

| 事项 | 为什么不是我能定的 |
|---|---|
| `--text-mute2` / `--text-mute3` 两个颜色的对比度 | 改颜色是设计决策，会影响全站观感 |
| #10 断点收敛 | 要么引入构建期预处理（加依赖），要么接受手工收敛的视觉回归风险 |

另有 4 个页面**明确不转区块**（`contact.html` / `gdpr.html` 两张表单、
`blog/index.html`、`404.html`），理由写在 `PLAN.md` 的阶段 3 一节。
