...
/* ---------------------------------------------------------------------------
   ...
   Same shape as page_versions: every save snapshots the full row, kept 50
   per article. Restore overwrites the live row + bookend snapshots.
--------------------------------------------------------------------------- */
const VERSION_KEEP = 50;
const ARTICLE_FIELDS_PLAIN = [
  'id', 'pillar_id', 'category_id', 'author_id', 'slug', 'title', 'excerpt', 'cover_url',
  'content', 'author', 'meta_title', 'meta_description', 'reading_minutes',
  'template', 'hero_image', 'published_at', 'status', 'created_at', 'updated_at',
].concat(SEO_FIELDS).join(', ');
...
router.post('/:id/versions/:vid/restore', requireAuth, async (req, res) => {
  ...
  // Slug is preserved on restore so inbound links stay alive. Status / dates
  // are restored from the snapshot. SEO columns aren't touched here — they
  // round-trip through the JSON snapshot already.
  await query(
