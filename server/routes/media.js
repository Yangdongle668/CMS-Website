...
async function buildVariants(filePath, mimetype) {
  ...
  try {
    ...
    for (const w of VARIANT_WIDTHS) {
      ...
      // WebP twin (better compression, supported by every modern browser).
...
// are silently skipped because the row may be older than the variant
// pipeline.
...
router.post('/', requireAuth, upload.single('file'), async (req, res) => {
  ...
  // takes ~600ms on commodity hardware — acceptable for a manual upload UX.
